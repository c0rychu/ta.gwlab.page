%% Chapter 1: Electrostatics  (textbook Topics 15--16)
%% Compilable on its own:  make latex CH=ch01-electrostatics
\documentclass[../book.tex]{subfiles}
% Resolve (and link) references into other chapters when the book has been
% built in this directory; latexmk keeps its .aux in build/, see .latexmkrc.
% Ignored entirely by the whole-book build, and a silent no-op if book.aux is
% absent -- so a stale path here costs cross-references with no error.
\IfFileExists{../build/book.aux}{\externaldocument{../build/book}}{}

\begin{document}

\chapter{Electrostatics}
\label{ch:electrostatics}

\section{Coulomb's law}

\begin{figure}[htbp]
  \centering
  \tikzfig{ch01-coulomb-law.tex}
  \caption{Coulomb's law. The source charge $Q$ sits at $\mathbf{r}'$ and the
  charge $q$ sits at $\mathbf{r}$, which feels the force $\mathbf{F}$ due to $Q$.}
  \label{fig:coulomb-law}
\end{figure}


\begin{keyeq}
  \label{eq:coulomb-law}
  \mathbf{F}
  =
  k_e 
  \frac{Q \, q}{\abs{\mathbf{R}}^2}
  \hat{\mathbf{R}}
  \;,
\end{keyeq}
where
\begin{align}
    k_e
    &=
    \frac{1}{4 \pi \epsilon_0}
    \approx 9 \times 10^9 \;\mathrm{N \cdot m^2 / C^2}
    \\
    \mathbf{R}
    &=
    \mathbf{r} - \mathbf{r}'
    \\
    \hat{\mathbf{R}}
    &=
    \frac{
    \mathbf{R}
    }{
    \abs{\mathbf{R}}
    }
    \;.
\end{align}



\section{The electric field}
Instead of having a Coulomb force between two charges $Q$ and $q$, we can have a different interpretation: The charge $Q$ does not interact with charge $q$ directly. Instead, the charge $Q$ creates an \emph{electric field} $\mathbf{E}(\mathbf{r})$ around it, and the charge $q$ at position $\mathbf{r}$ feels a force from the electric field $\mathbf{E}(\mathbf{r})$ locally at $\mathbf{r}$. 

The electric field $\mathbf{E}$ is a \emph{vector field}. We may think of a vector field as: ``at every point in the space $\mathbf{r}$, there is a vector $\mathbf{V}(\mathbf{r})$ (a little arrow) attached to that point $\mathbf{r}$.'' Because of that, a vector field $\mathbf{V}(\mathbf{r})$ is a function of location, i.e., space coordinate, $\mathbf{r}=(x,y,z)$.

\begin{figure}[htbp]
  \centering
  \tikzfig{ch01-coulomb-law.tex}
  \caption{Same as \autoref{fig:coulomb-law}}
  \label{fig:coulomb-law-E-field}
\end{figure}

To make our two-charge example more explicit, we can refer to \autoref{fig:coulomb-law-E-field}. Here, charge $Q$ creates an electric field:
\begin{align}
    \label{eq:coulomb-law-E-field}
    \mathbf{E}(\mathbf{r})
    &=
    k_e 
    \frac{Q}{\abs{\mathbf{R}}^2}
    \hat{\mathbf{R}}
    \\
    &=
    k_e 
    \frac{Q}{\abs{\mathbf{r} - \mathbf{r}'}^2}
    \frac{\mathbf{r} - \mathbf{r}'}{\abs{\mathbf{r} - \mathbf{r}'}}
    \;,
\end{align}
and change $q$ feels the force
\begin{align}
    \mathbf{F}(\mathbf{r})
    &=
    q \mathbf{E}(\mathbf{r})
    \\
    &=
    k_e 
    \frac{Q \, q}{\abs{\mathbf{R}}^2}
    \hat{\mathbf{R}}
    \;.
\end{align}
Quantitatively, this is the same as the Coulomb force in the static case, and it seems the $\mathbf{E}$ field is just a mathematical tool to facilitate calculation. However, this is in fact a deep breakthrough in theory, which later on leads to the prediction of the existence of electromagnetic waves.


\section{Gauss's law}

\begin{figure}[htbp]
    \centering
    \tikzfig{ch01-gauss-surface-arbitrary.tex}
    \caption{A Gaussian surface $S$ of arbitrary shape enclosing the charge $Q$.
    At each point of $S$, the electric field $\mathbf{E}$ meets the area element
    $d\mathbf{A}$ at an angle $\theta$ that varies over the surface. Even so,
    every electric field line crosses $S$, so the total flux through $S$ is
    $Q/\epsilon_0$.}
    \label{fig:gauss-surface-arbitrary}
\end{figure}

\begin{gwbox}[Gauss's law]
The total electric flux $\Phi_E$ through any \emph{closed} surface $S$, known as a Gaussian surface, equals the total charge $Q_{\mathrm{enc}}$ enclosed by $S$ divided by $\epsilon_0$:
\begin{keyeq}
    \label{eq:gauss-law}
    \Phi_E
    =
    \oint_S \mathbf{E} \cdot d\mathbf{A}
    =
    \frac{Q_{\mathrm{enc}}}{\epsilon_0}
    \;.
\end{keyeq}
\begin{itemize}
    \item
        The surface integral $\oint_S$ means ``add up over the whole closed surface''. On each small patch, $d\mathbf{A}$ points outward, and $\mathbf{E} \cdot d\mathbf{A} = E \, dA \cos\theta$ counts only the part of $\mathbf{E}$ that passes through the patch.
    \item
        Only the charge \emph{inside} $S$ counts. A charge outside sends as many field lines in through $S$ as out, so the net flux due to an outside charge is zero.
\end{itemize}
\end{gwbox}

Since Gauss's law holds for a closed Gaussian surface of any shape, a clever choice of the surface is the key to simplifying the problem.

\begin{figure}[htbp]
    \centering
    \tikzfig{ch01-gauss-surface-sphere.tex}
    \caption{A spherical Gaussian surface $S'$ of radius $R$ centered on the
    charge $Q$. Everywhere on $S'$, $\mathbf{E}$ is parallel to $d\mathbf{A}$
    and has the same magnitude.}
    \label{fig:gauss-surface-sphere}
\end{figure}

\begin{align}
    \Phi_E
    &=
    \frac{Q_{\mathrm{enc}}}{\epsilon_0}
    \\
    \Rightarrow
    E A
    &=
    E (4 \pi R^2)
    =
    \frac{Q_{\mathrm{enc}}}{\epsilon_0}
    \\
    \Rightarrow
    E
    &=
    \frac{1}{4 \pi \epsilon_0}
    \frac{ Q_{\mathrm{enc}} }{R^2}
\end{align}

For a pair of infinitely large parallel plates in vacuum:
\begin{align}
    \Phi_E
    &=
    E A
    =
    \frac{\sigma A}{\epsilon_0}
    \\
    \Rightarrow
    E
    &=
    \frac{\sigma}{\epsilon_0}
    \;,
\end{align}
where $\sigma \equiv \tfrac{Q}{A}$ is the surface charge density.

\section{Electric potential energy}
\begin{align}
    W = \mathbf{F} \cdot \Delta \mathbf{ x}
\end{align}
\begin{align}
    \Delta V = \mathbf{E} \cdot \Delta \mathbf{ x}
\end{align}

For a point charge $Q$:
\begin{align}
    V =
    \frac{1}{4 \pi \epsilon_0}
    \frac{Q}{R}
    \quad
    (\text{when choosing $V=0$ at $R \rightarrow \infty$})
\end{align}


\section{Capacitance and dielectrics}
\begin{align}
    C \equiv
    \frac{\Delta Q}{\Delta V}
\end{align}
For a parallel-plate Capacitor,
\begin{align}
    C =
    \epsilon
    \frac{A}{d}
\end{align}



\chapterbib

\end{document}
